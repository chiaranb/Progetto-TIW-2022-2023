...
public class CategoryDAO {
		...
		try (PreparedStatement pstatement = connection
			...
			try (ResultSet resultSet = pstatement.executeQuery();) {
				if (resultSet.next() && resultSet.getInt(1) == 0) {
					throw new IllegalArgumentException("Father category does not exist");
				}
			}
		}
	...
	public void copyHereSubtree(int id, int fid) throws SQLException {
		...
		try (PreparedStatement pstatementCount = connection.prepareStatement(
				"SELECT COUNT(*) FROM catalog.category WHERE father = ?")) {
		...
		String query1 = "SELECT name, creator FROM catalog.category WHERE id = ?";
	...
	public void modifyName(String name, int id) throws SQLException {
		try (PreparedStatement pstatement = connection
				.prepareStatement("UPDATE catalog.category SET name = ? WHERE id = ?");) {
			pstatement.setString(1, name);
			pstatement.setInt(2, id);
			int rowUpdated = pstatement.executeUpdate();
			if (rowUpdated == 0) {
				throw new IllegalArgumentException("Id category does not exist");
			}
		}
	}
}
